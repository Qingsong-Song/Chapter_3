...
class LagosWrightAiyagariSolver:
    ...
    def firm_problem(self, prices):
        """
        Compute firm-side decisions and total illiquid supply for the illiquid asset market.
                ...
                for f_idx, a_f in enumerate(self.f_grid):
                    ...
                    for m_idx, a_m in enumerate(self.m_grid):
                        ...
                        if effective_max_borrow > 0:
                            ...
                            l0 = np.clip(py * y0_b - a_m, 0,
                                         effective_max_borrow)  # borrow exactly the amount needed to consume
                    ...
                    for b_idx, b in enumerate(self.b_grid):
                        ...
                        for e_next_idx in range(self.n_e):
                            ...
                            b_noshock = (m_wi * f_wi * policy_b_noshock[m_lower_idx, f_lower_idx, z_idx, e_next_idx] +
                                         (1 - m_wi) * f_wi * policy_b_noshock[
                                             m_upper_idx, f_lower_idx, z_idx, e_next_idx] +
                                         m_wi * (1 - f_wi) * policy_b_noshock[
                                             m_lower_idx, f_upper_idx, z_idx, e_next_idx] +
                                         (1 - m_wi) * (1 - f_wi) * policy_b_noshock[
                                             m_upper_idx, f_upper_idx, z_idx, e_next_idx])

                            # Calculate the next state
                            # 1: No shock (ϵ = 0)
                            a_next_noshock = total_asset - b_noshock
                            a_noshock_lower, a_noshock_upper, a_noshock_wt = find_closest_indices(self.a_grid,
                                                                                                  a_next_noshock)
                            b_noshock_lower, b_noshock_upper, b_noshock_wt = find_closest_indices(self.b_grid,
                                                                                                  b_noshock)
                            ...
                            G_new[a0_next_lower, b0_lower, z_idx, e_next_idx] += (
                                        g * self.alpha * self.alpha_0 * a0_wt * b0_wt * prob)
        ...
        while market_iter < max_iter_market:
            ...
            while iteration < self.max_iter and not converged:
                ...
                if iteration % report_frequency == 0 or converged:
                    print(
                        f"[VF] Iter {iteration}: Max diff = {max_diff:.2e}, Mean diff = {mean_diff:.2e}, Time = {iter_time:.2f}s")
            ...
            self.export_4d_to_excel(dm_output['policy_b_noshock'], 'exports/policy_b_noshock.xlsx',
                                    sheet_name='BorrowingNoshock')
            print("Export complete.")
            ...
            print(
                f"Excess demand: Goods = {excess_demand[0]:.2e}, Illiquid assets = {excess_demand[1]:.2e}, Bank = {excess_demand[2]:.2e}")
            print(
                f"Demand: Goods = {demand_vector[0]:.2e}, Illiquid assets = {demand_vector[1]:.2e}, Bank = {demand_vector[2]:.2e}")
            print(
                f"Supply: Goods = {supply_vector[0]:.2e}, Illiquid assets = {supply_vector[1]:.2e}, Bank = {supply_vector[2]:.2e}")
...
# --- Plot Distribution Convergence ---Matthew Easton
