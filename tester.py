...
class LagosWrightAiyagariSolver:
    def __init__(self, params):
        ...
        lambda_value = self.job_finding(self.market_tightness)
            ...
            for z_idx in range(self.n_z):
                ...
                for a_idx in range(self.n_a):
                    ...
                    for D_idx in range(self.n_D):
                        D = self.D_grid[D_idx]
                        # Initial W value
                        inc = z * (self.w1 if e_idx == 1 else self.w0)  # Income based on skill and employment
    ...
    def solve_cm_problem(self, e_idx, a_idx, D_idx, z_idx):
        ...
        Parameters:
        e_idx: Index of employment state
        a_idx: Index of asset holdings
        ...
        # Add income based on employment and skill
        if e_idx == 1:  # Employed
            resources += z * self.w1
        else:  # Unemployed
            resources += z * self.w0
        ...
        if D > 0:  # Deposit
            ...
            resources += phi_m * (1 + i) * D  # D is negative, so this subtracts the repayment
        
        # Grid of possible portfolio choices (can be made more efficient)
...
def utility(self, c):
    """Utility function for centralised market"""
    if c <= 0:
