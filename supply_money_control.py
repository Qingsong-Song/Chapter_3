...
class LagosWrightAiyagariSolver:
    def __init__(self, params):
        ...
        self.ϖ = params['ϖ']  # Probability of meeting a bank with redudant cash
    ...
    def solve_dm_problem_vectorised(self, W_guess, prices, firm_result):
        ...
        policy_b_noshock = np.zeros(shape, dtype=np.float32)   # experiment: no shock and bank
        policy_b_nobank = np.zeros(shape, dtype=np.float32)    # experiment: no shock and no bank --> deposite is zero
    ...
    def household_transition(self, G_guess, prices, cm_output, dm_output, firm_result):
        ...
        # policy_b_nobank = dm_output['policy_b_nobank']
            ...
            for z_idx in range(self.n_z):
                ...
                for a_idx in range(self.n_a):
                    for b_idx in range(self.n_b):
                        ...
                        # find the total asset & borrowing indices:
                        a_nobank_lower, a_nobank_upper, a_nobank_wt = find_closest_indices(self.a_grid, total_asset)
                        b_nobank_lower, b_nobank_upper, b_nobank_wt = find_closest_indices(self.b_grid, 0)
                        ...
                        for e_next_idx in range(self.n_e):
                            ...
                            G_new[a_noshock_lower, b_noshock_lower, z_idx, e_next_idx] += (g * (1 - self.alpha) * self.ϖ *
                                                                                            a_noshock_wt * b_noshock_wt * prob)
                            G_new[a_noshock_upper, b_noshock_lower, z_idx, e_next_idx] += (g * (1 - self.alpha) * self.ϖ *
                                                                                            (1 - a_noshock_wt) * b_noshock_wt * prob)
                            G_new[a_noshock_lower, b_noshock_upper, z_idx, e_next_idx] += (g * (1 - self.alpha) * self.ϖ *
                                                                                            a_noshock_wt * (1 - b_noshock_wt) * prob)
                            G_new[a_noshock_upper, b_noshock_upper, z_idx, e_next_idx] += (g * (1 - self.alpha) * self.ϖ *
                                                                                            (1 - a_noshock_wt) * (1 - b_noshock_wt) * prob)
                            # 1.2: No shock (ϵ = 0) + no bank
                            G_new[a_nobank_lower, b_nobank_lower, z_idx, e_next_idx] += (g * (1 - self.alpha) * (1 - self.ϖ) *
                                                                                         a_nobank_wt * b_nobank_wt * prob)
                            G_new[a_nobank_upper, b_nobank_lower, z_idx, e_next_idx] += (g * (1 - self.alpha) * (1 - self.ϖ) *
                                                                                         (1 - a_nobank_wt) * b_nobank_wt * prob)
                            G_new[a_nobank_lower, b_nobank_upper, z_idx, e_next_idx] += (g * (1 - self.alpha) * (1 - self.ϖ) *
                                                                                         a_nobank_wt * (1 - b_nobank_wt) * prob)
                            G_new[a_nobank_upper, b_nobank_upper, z_idx, e_next_idx] += (g * (1 - self.alpha) * (1 - self.ϖ) *
                                                                                         (1 - a_nobank_wt) * (1 - b_nobank_wt) * prob)
                ...
                for m_idx in range(self.n_m):
                    for f_idx in range(self.n_f):
                        for e_next_idx in range(self.n_e):
                            ...
                            b_noshock = policy_b_noshock[m_idx, f_idx, z_idx, e_next_idx]
                            b_nobank = policy_b_nobank[m_idx, f_idx, z_idx, e_next_idx]
                            ...
                            # experiment: demand from the no bank scenario is 0 so not included
                            Bd += f_mass * (self.alpha * (self.alpha_0 * Bd_0 + self.alpha_1 * 
                                        Bd_1) + (1 - self.alpha) * (self.ϖ * Bd_noshock + 
                                                                    (1 - self.ϖ) * Bd_nobank)) * P[e_idx, e_next_idx]
                            Bs += f_mass * (self.alpha * (self.alpha_0 * Bs_0 + self.alpha_1 * 
                                        Bs_1) + (1 - self.alpha) * (self.ϖ * Bs_noshock + 
                                                                    (1 - self.ϖ) * Bs_nobank)) * P[e_idx, e_next_idx]
    ...
    def solve_model(self, prices=None, plot_frequency=50, report_frequency=100, tol_dist=1e-5, 
        ...
        while market_iter < max_iter_market:
            ...
            dm_output = {
                ...
                'policy_b_nobank': results['dm_results']['policy_b_nobank']
            }
            ...
            self.export_4d_to_excel(dm_output['policy_b_nobank'], 'exports/policy_b_nobank.xlsx', sheet_name='BorrowingNoBank')
            print("Export complete.")
